...
import { type List, type WorkspaceContext } from "../types";
...
export async function getWorkspaceContext(params: {
  ...
  // `kind` é text com CHECK no banco, então o type gerado o expõe como
  // `string`; `List` estreita para a união real, como já se faz em `Item`.
  const allLists = listsRes.data as List[];

  const requestedList = params.listId
    ? allLists.find((list) => list.id === params.listId)
    : undefined;
  ...
  const lists = allLists.filter((list) => list.workspace_id === workspace.id);
